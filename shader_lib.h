...
{
    static_assert(areSwizzlersUnique({ X, Y }));
    m[X] = v.x, m[Y] = v.y;
    return *this;
}
...
{
    static_assert(areSwizzlersUnique({ X, Y, Z }));
...
{
    static_assert(areSwizzlersUnique({ X, Y, Z, W }));
    m[X] = v.x, m[Y] = v.y, m[Z] = v.z;
    return *this;
}
...
{ static_assert(areSwizzlersUnique({ X, Y, Z, W })); m[X] op##= v.x, m[Y] op##= v.y, m[Z] op##= v.z, m[W] op##= v.w; return *this; }
...
FORCEINLINE Vector3_base saturate(const Vector3_base& a) { return Vector3_base(CLAMP_IMPL(a.x, 0.f, 1.f), CLAMP_IMPL(a.y, 0.f, 1.f), CLAMP_IMPL(a.z, 0.f, 1.f)); }
...
{
    t = clamp((t - edge0) / (edge1 - edge0), 0.f, 1.f);
...
FORCEINLINE Vector2_base<float> sin(const Vector2_base<float>& v) { return Vector2_base<float>(std::sin(v.x), std::sin(v.y)); }
FORCEINLINE Vector3_base<float> sin(const Vector3_base<float>& v) { return Vector3_base<float>(std::sin(v.x), std::sin(v.y), std::sin(v.z)); }
FORCEINLINE Vector4_base<float> sin(const Vector4_base<float>& v) { return Vector4_base<float>(std::sin(v.x), std::sin(v.y), std::sin(v.z), std::sin(v.z)); }

FORCEINLINE float cos(const float v) { return std::cos(v); }
FORCEINLINE Vector2_base<float> cos(const Vector2_base<float>& v) { return Vector2_base<float>(std::cos(v.x), std::cos(v.y)); }
FORCEINLINE Vector3_base<float> cos(const Vector3_base<float>& v) { return Vector3_base<float>(std::cos(v.x), std::cos(v.y), std::cos(v.z)); }
FORCEINLINE Vector4_base<float> cos(const Vector4_base<float>& v) { return Vector4_base<float>(std::cos(v.x), std::cos(v.y), std::cos(v.z), std::cos(v.z)); }
...
struct hash<Vector2_base<T>> {
    std::size_t operator()(const Vector2_base<T>& v) const
    {
        size_t seed = 0;
...
struct hash<Vector3_base<T>> {
    std::size_t operator()(const Vector3_base<T>& v) const
    {
        size_t seed = 0;
...
struct hash<Vector4_base<T>> {
    std::size_t operator()(const Vector4_base<T>& v) const
    {
        size_t seed = 0;
